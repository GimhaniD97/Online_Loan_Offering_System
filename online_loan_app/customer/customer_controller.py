...
def add_new_customer(first_name, last_name, user_name, user_password, email, dob, mobile_no):
    ...
    logging = logger.info('customer_controller', log_id)
    try:
        ...
        raise Exception("Internal server error - add new customer from database {}".format(e))
...
def validate_customer(access_token):
    logger.info('user_controller', log_id)
    try:
        ...
        logger.error('[{}]'.format(str(e)))
        raise Exception('Internal Server Error - Token Verification {}'.format(e))
